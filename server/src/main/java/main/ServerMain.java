    ...
    public static void run(String[] args) {
        try {
            try {
                ...
                ServerMain.logger.error("Критическая ошибка старта: {}", e.getMessage());
                System.exit(0);
            }
            ...
            logger.info("UDP модуль инициализирован на порту {}", port);

            Runtime.getRuntime().addShutdownHook(new Thread(() -> {
                logger.info("Остановка сетевого пайплайна...");
                pipeline.stop();
            }, "Pipeline-Shutdown-Thread"));
            ...
            logger.info("Начальная runtime-конфигурация: port={} ({}), packet_capacity={} ({}), socket_timeout_ms={} ({}), socket_receive_buffer={} ({})",
                    appliedConfig.serverPort(), appliedConfig.serverPortSource(),
                    appliedConfig.packetCapacityBytes(), appliedConfig.packetCapacitySource(),
                    appliedConfig.socketTimeoutMs(), appliedConfig.socketTimeoutSource(),
                    appliedConfig.socketReceiveBufferBytes(), appliedConfig.socketReceiveBufferSource());

            pipeline.start(connectionModule.socket());
            logger.info("Сервер успешно запущен: пайплайн принимает UDP-запросы.");
            ...
            logger.error("Критическая ошибка server pipeline", e);
        }
    }
